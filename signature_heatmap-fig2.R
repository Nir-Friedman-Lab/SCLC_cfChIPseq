...
s = c(s.samples, sl.samples, h.samples, l.samples, c.samples)
...
test.groups = c("SCLC", "SCLC-like", "Healthy", "NSCLC", "CRC")
...
if (F) {
  ...
  heatmap_legend_param = c(annotation.params, 
                           list(title = "log2 (1 + reads)",
                                at = scale_range,
                                labels = scale_label))
  col.a = HeatmapAnnotation(df = sample.annotation[colnames(d),, drop = F], 
                           col = group.colors,  
  ...
  subtype.a = HeatmapAnnotation(df = sample.annotation[colnames(d),, drop = F],
                                annotation_label = "",
                                show_legend = F,
                                simple_anno_size = unit(2, "mm"),
                                annotation_name_gp = gp,
                                foo = anno_block(height = unit(15, "pt"),
                                                 labels = c("SCLC", "SCLC-like", 
                                                            "Healthy", "NSCLC", "CRC"),
                                                 gp = gpar(lwd = 0),
                                                 # gp = gpar(fill = c(group.colors$$subtype[1:5], "NA" = "grey"), lwd = 0),
                                                 labels_gp = gp))
  pdf(file = paste0(figDirPaper, "figure2/signature_heatmap_V1.pdf"), width = 4.7, 
      height = 2.5)
  ...
  Heatmap(d, col = cfm.color, 
          ...
          column_split = factor(sample.annotation[colnames(d), "group"], 
                                levels = c("SCLC", "SCLC-like", "Healthy", "NSCLC", "CRC")),
...
comparisons = list(c("SCLC", "SCLC-like"), c("SCLC", "Healthy"), 
